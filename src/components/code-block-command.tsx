...
import { IconAlignLeft, IconTerminal } from "@tabler/icons-react";
...
export function CodeBlockCommand({
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-xl bg-code", className)}>
      <Tabs
        className="gap-0"
      ...
      >
        <ScrollArea.Root className="w-full border-b border-dashed pr-10">
          <TabsList
            variant="line"
            className={cn(
              "dmax-w-full h-10! justify-start rounded-none bg-transparent pl-4 inset-ring-0 dark:bg-transparent [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:text-muted-foreground",
          ...
          >
            ...
            {tabsFiltered.map(([key]) => {
              return (
                <TabsTrigger key={key} className="p-0 px-2 font-mono" value={key}>
                  {key}
                </TabsTrigger>
              );
            })}
